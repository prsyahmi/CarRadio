...
void nmea_reader_parse(char *line) {
	switch (minmea_sentence_id(line, false)) {
		...
		case MINMEA_INVALID: {
			D("%s sentence is not valid\n", line);
		} break;

		default: {
			if (memcmp(line, "$BTNPR,up", 9) == 0) {
			   D("up button pressed\n");
			   emitKey(EV_KEY, KEYCODE_MEDIA_NEXT, 1);
			   emitKey(EV_SYN, SYN_REPORT, 1);
			   emitKey(EV_KEY, KEYCODE_MEDIA_NEXT, 0);
			   emitKey(EV_SYN, SYN_REPORT, 0);
			} else if (memcmp(line, "$BTNPR,dn", 9) == 0) {
			   D("down button pressed\n");
			   emitKey(EV_KEY, KEYCODE_MEDIA_PREVIOUS, 1);
			   emitKey(EV_SYN, SYN_REPORT, 1);
			   emitKey(EV_KEY, KEYCODE_MEDIA_PREVIOUS, 0);
			   emitKey(EV_SYN, SYN_REPORT, 0);
			} else if (memcmp(line, "$BTNPR,vol+", 11) == 0) {
			   D("vol+ button pressed\n");
			   emitKey(EV_KEY, KEY_VOLUMEUP, 1);
			   emitKey(EV_SYN, SYN_REPORT, 1);
			   emitKey(EV_KEY, KEY_VOLUMEUP, 0);
			   emitKey(EV_SYN, SYN_REPORT, 0);
			} else if (memcmp(line, "$BTNPR,vol-", 11) == 0) {
			   D("vol- button pressed\n");
			   emitKey(EV_KEY, KEY_VOLUMEDOWN, 1);
			   emitKey(EV_SYN, SYN_REPORT, 1);
			   emitKey(EV_KEY, KEY_VOLUMEDOWN, 0);
			   emitKey(EV_SYN, SYN_REPORT, 0);
			} else if (memcmp(line, "$BTNPR,back", 11) == 0) {
			   D("back button pressed\n");
			   emitKey(EV_KEY, KEY_ESC, 1);
			   emitKey(EV_SYN, SYN_REPORT, 1);
			   emitKey(EV_KEY, KEY_ESC, 0);
			   emitKey(EV_SYN, SYN_REPORT, 0);
			} else if (memcmp(line, "$BTNPR,test", 11) == 0) {
